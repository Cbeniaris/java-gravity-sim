...
//		System.out.println(force);             // non-zero vector pulling a toward center
		...
		System.out.println("Begin: QuadTree class test");
		QuadTree tree = new QuadTree(0.5, 100);
		...
		Vector2D forceOnA = tree.calculateForce(a, 6.674e-11, 0.1);
		Vector2D forceOnB = tree.calculateForce(b, 6.674e-11, 0.1);

		System.out.println(forceOnA); // positive x, negative y
		System.out.println(forceOnB); // negative x, positive y — mirror of A

		// Forces on A and B should be equal and opposite in x and y magnitude
		System.out.println(Math.abs(forceOnA.x) - Math.abs(forceOnB.x) < 1e-6); // true
	}
}
